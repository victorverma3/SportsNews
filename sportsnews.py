...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar  5 20:05:59 2023
...
seasonal = list(filter(lambda x: x not in year, sports.keys())) # designated seasonal sports
times = {
    'cbasketball': {'start': date(2023, 11, 1),
                    'end': date(2024, 4, 10)},
    'cfootball': {'start': date(2023, 8, 20),
                  'end': date(2024, 1, 15)}
    }
...
def sendEmail(sport, emails):
    ...
    with smtplib.SMTP("smtp-mail.outlook.com", 587) as s: # establishes a connection to the hotmail server
...
if os.environ.get('call') == '1':
    if __name__ == '__main__':
        ...
        for sport in seasonal: #sends emails to designated seasonal sports if within season
